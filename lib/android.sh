...
flenv_provision_android() {
	...
	flenv_step "Downloading Android CLI"
	...
	"$sdk/platform-tools/adb" version >/dev/null || flenv_die "adb validation failed"
	HOME="$cli_home" JAVA_HOME="$java_home" "$android" --sdk="$sdk" sdk list --all >/dev/null 2>&1 ||
